...
def main():
    for percent_history_year in [5,6,7,8,9,10]:
        for select_num in [1,2,3,4,5,7,10]:
            ...
            back_df = pp.back_df
            back_df.to_excel('DataSave\\PEPB研究\\n年m指数回测原始数据\\'+'history={}year&num={}_原始数据.xlsx'.format(percent_history_year,select_num))
            ra = ResultAnalysis(back_df)
            ra.sta_once_df.to_excel('DataSave\\PEPB研究\\n年m指数回测净值数据\\'+'history={}year&num={}.xlsx'.format(percent_history_year,select_num))
...
#sta_many_df.to_excel('n年m个指数.xlsx')
